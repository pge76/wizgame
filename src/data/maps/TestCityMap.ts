...
function drawVerticalWall(rows: string[][], x: number, y0: number, y1: number, doorEvery: number): void {
  for (let y = y0; y <= y1; y++) {
    if ((y - y0) % doorEvery === Math.floor(doorEvery / 2)) continue;
    rows[y]![x] = "#";
  }
}

function drawHorizontalWall(rows: string[][], y: number, x0: number, x1: number, doorEvery: number): void {
  for (let x = x0; x <= x1; x++) {
    if ((x - x0) % doorEvery === Math.floor(doorEvery / 2)) continue;
    rows[y]![x] = "#";
  }
}
...
function buildRows(): string[] {
  ...
  // Gates through the outer wall (approximating the arrow exits in the reference image).
  rows[24]![wallX0] = ".";
  rows[25]![wallX0] = ".";
  rows[20]![wallX1] = ".";
  rows[wallY1]![27] = ".";
  ...
  for (const x of [16, 24, 32, 40]) {
    drawVerticalWall(rows, x, wallY0 + 1, wallY1 - 1, 6);
  }
  // Internal room grid - horizontal dividers with doorway gaps.
  for (const y of [15, 22, 34]) {
    drawHorizontalWall(rows, y, wallX0 + 1, wallX1 - 1, 8);
  }
